...
SAMPLE_SITES = [
    dict(name="福島県 郡山市 工業団地隣接地", address="福島県郡山市田村町", prefecture="福島県",
    ...
    dict(name="北海道 苫小牧市 工業専用地域", address="北海道苫小牧市勇払", prefecture="北海道",
    ...
    dict(name="愛知県 豊橋市 工業専用地域", address="愛知県豊橋市神野新田", prefecture="愛知県",
    ...
    dict(name="岡山県 倉敷市 準工業地域", address="岡山県倉敷市玉島", prefecture="岡山県",
    ...
    dict(name="茨城県 鉾田市 農地転用候補", address="茨城県鉾田市大竹", prefecture="茨城県",
    ...
    dict(name="熊本県 菊池市 農地（第2種）", address="熊本県菊池市泗水町", prefecture="熊本県",
    ...
    dict(name="宮城県 石巻市 沿岸工業地", address="宮城県石巻市渡波", prefecture="宮城県",
    ...
    dict(name="静岡県 焼津市 第1種農地", address="静岡県焼津市大覚寺", prefecture="静岡県",
...
def seed():
    ...
    try:
        if db.query(models.Site).count() > 0:
            print("既にデータが存在します。既存データを更新します。")
            for data in SAMPLE_SITES:
                site = db.query(models.Site).filter_by(name=data["name"]).first()
                if site:
                    site.prefecture = data["prefecture"]
            db.commit()
            print("都道府県情報を更新しました。")
            return

        for data in SAMPLE_SITES:
            db.add(models.Site(**data))
